...
{
  if (HAL_OSPI_DeInit(ospi) != HAL_OK)
    return HAL_ERROR;

  MX_OCTOSPI1_Init();

  if (W25Q64_ResetChip(ospi) != HAL_OK)
    return HAL_ERROR;

  if (W25Q64_Configuration(ospi) != HAL_OK)
    return HAL_ERROR;

  HAL_Delay(1);

  if (W25Q64_AutoPollingMemReady(ospi) != HAL_OK)
    return HAL_ERROR;

  if (W25Q64_WriteEnable(ospi) != HAL_OK)
    return HAL_ERROR;
...
W25Q64_ResetChip (OSPI_HandleTypeDef *ospi)
{
  ...
  if (HAL_OSPI_Command (ospi, &cmd, HAL_OSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK)
    return HAL_ERROR;
...
W25Q64_Configuration (OSPI_HandleTypeDef *ospi)
{
  ...
  if (W25Q64_Read_Status_Registers(ospi, &reg1, 1) != HAL_OK)
    return HAL_ERROR;

  if (W25Q64_Read_Status_Registers(ospi, &reg2, 2) != HAL_OK)
    return HAL_ERROR;

  if (W25Q64_Read_Status_Registers(ospi, &reg3, 3) != HAL_OK)
    return HAL_ERROR;
  ...
  if (W25Q64_Write_Status_Registers(ospi, w_reg1, 1) != HAL_OK)
    return HAL_ERROR;

  if (W25Q64_Write_Status_Registers(ospi, w_reg2, 2) != HAL_OK)
    return HAL_ERROR;

  if (W25Q64_Write_Status_Registers(ospi, w_reg3, 3) != HAL_OK)
    return HAL_ERROR;
...
W25Q64_WriteEnable (OSPI_HandleTypeDef *ospi)
{
  ...
  cfg.Interval  = W25Q_AUTOPOLLING_INTERVAL_TIME;
  cfg.AutomaticStop = HAL_OSPI_AUTOMATIC_STOP_ENABLE;

  if (HAL_OSPI_AutoPolling (ospi,&cfg,HAL_OSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK)
    return HAL_ERROR;
...
W25Q64_Erase_Chip (OSPI_HandleTypeDef *ospi)
{
  ...
  while (W25Q64_IsBusy(ospi) == HAL_ERROR)
    HAL_Delay(1);
  ...
  do
    {
      ...
      if (HAL_OSPI_Transmit (ospi, (uint8_t*)data_addr,
             ...
             uint32_t             size)
{
  ...
  if (HAL_OSPI_Receive (ospi, data, HAL_OSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK)
    return HAL_ERROR;
...
{
  OSPI_RegularCmdTypeDef cmd = {0};
  OSPI_MemoryMappedTypeDef mapped_cfg = {0};
  ...
  mapped_cfg.TimeOutActivation = HAL_OSPI_TIMEOUT_COUNTER_DISABLE;

  if (HAL_OSPI_MemoryMapped (ospi, &mapped_cfg) != HAL_OK)
    return HAL_ERROR;
...
W25Q64_IsBusy (OSPI_HandleTypeDef *ospi)
{
  ...
  state = W25Q64_Read_Status_Registers (ospi, &status_rgister, 1);
  if (state != HAL_OK)
    return state;
                              ...
                              uint8_t              reg_num)
{
  ...
  if(HAL_OSPI_Receive(ospi, reg_data, HAL_OSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK)
    return HAL_ERROR;
                               ...
                               uint8_t              reg_num)
{
  ...
  if(HAL_OSPI_Transmit(ospi, &reg_data, HAL_OSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK)
    return HAL_ERROR;
